...
Imagine you are working on the application, and you leave your API key visible in your code. If you were to push your code to GitHub, someone could find your repo and copy your API key so they can make requests using it. This would then make you responsible for the calls that this person made. If an API uses a key to limit daily requests, this other person might reach your allotted number of requests. This means your app couldn't make any more requests to get the data it needs to run correctly. Even worse, if the API key is tied to a credit card charged for each request made, you could owe a lot of money. Even if you don't push your code to GitHub, if your app is deployed, there are other ways hackers could figure out your API Key.
...
The easiest way to hide your API keys is to create a "proxy server," just a web application requesting the API. Here's how the flow of requests/responses would go:
...
Start by running `nodemon` in the terminal and test out `http://localhost:3000/` in the browser. You should see "Cannot GET /"
...
If you go to `http://localhost:3000/` in the browser, you should see the data your proxy server retrieves from the API.

## Adjust the `fetch` URL

You might notice I've hard-coded the city we're querying as `Philadelphia`. This is just a random choice I made for testing purposes. Of course, we don't want to only query for `Philadelphia`. Let's make it so that the user can specify any city by assigning a value to a query param called `city`:
...
Now, we can get results for any city that we query. `http://localhost:3000/?city=Raleigh` will search for  Raleigh, `http://localhost:3000/?city=Philadelphia` will search for Philadelphia, and so on.
...
Note that your React app is running on port `5173` and your new proxy server is on port `3000`. This is good! Each port can only have one application listening on it. The problem is that browsers consider it insecure to make AJAX requests to other domains or ports (this is called Cross-Origin Resource Sharing or CORS). So even though both React and your proxy server are accessed via `localhost` because they're on different ports, having your React app make an AJAX request to your proxy server is considered insecure. This is easily fixed, though!
...
We're almost at the end! The next step is hiding our API key, which is still visible in our Express code. Need to be more secure! Let's hide it in a `.env` file. First, install the `dotenv` package:
